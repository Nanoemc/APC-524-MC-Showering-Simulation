...
def simple_shower_alg(inshower: Shower, init_e: float):
    gen = 0
    shower_max = log(init_e / inshower.shower_surface().e_crit()) / log(
        2
    )  # Max depth of shower in radiation lengths (See Tavernier page 193)
...
if __name__ == "__main__":
    giant_block_of_polystyrene = Material(100 * 2000, "")
    new_shower_1 = Shower(
        surface=giant_block_of_polystyrene,
        initial_e=10000,
        verbose=True,
        write_to_file=True,
    )  # 10 GeV electron
